...
# Build nodes (unique 'Grade: Class' labels)
...
# Map source/target to their indices
...
# --- Assign colors by class type ---
# 1. Extract class type from the node name (e.g., "Kindergarten: small" -> "small")
get_class <- function(x) sub(".*: ", "", x)

nodes_df$class_type <- sapply(nodes_df$name, get_class)

# 2. Define a color palette matching your desired look
class_colors <- c(
  "small"        = "#F5C310",  # yellow
  "regular"      = "#0072B2",  # blue
...
# 3. Map each node to the appropriate color
nodes_df$color <- class_colors[nodes_df$class_type]

# For the links, color them by the source node’s class (so flows keep the same color as the source)
...
p <- plot_ly(
  type = "sankey",
  orientation = "h",
  # "snap" places nodes in vertical columns by shared x-coordinates
  arrangement = "snap",
  node = list(
    label = nodes_df$name,
    pad = 15,
...
```{r, warning=FALSE}
# --- Data Preparation ---
# 1) Subset only the relevant columns
# 2) Remove rows that have any NA values
# 3) Convert numeric codes to factor levels
...
# Create flows between consecutive grades
pairs <- list(c("gkclasstype", "g1classtype"),
              c("g1classtype", "g2classtype"),
              c("g2classtype", "g3classtype"))
...
# --- Assign colors by class type ---
# Extract the class type from each node name (e.g., "Kindergarten: small" -> "small")
get_class <- function(x) sub(".*: ", "", x)
nodes_df$class_type <- sapply(nodes_df$name, get_class)

# Define a color palette for each class type
class_colors <- c(
  "small"        = "#F5C310",  # yellow
  "regular"      = "#0072B2",  # blue
  "regular-aide" = "#009E73"   # green
...
# Assign node colors based on class type
nodes_df$color <- class_colors[nodes_df$class_type]

# For link colors, match the source node’s color
transitions$link_color <- scales::alpha(nodes_df$color[ match(transitions$source, nodes_df$name) ], 0.4)
...
p <- plot_ly(
  type = "sankey",
  orientation = "h",
  arrangement = "snap",  # places nodes in columns
  node = list(
    label = nodes_df$name,
    pad = 15,
    thickness = 20,
...
## --- Pie Chart 1: Achievement Data Completeness ---
...
## --- Pie Chart 2: High School Data Completeness ---
...
## --- Pie Chart 3: Achievement & High School Data Completeness ---
...
## --- Create a 1x3 layout and plot the pie charts ---
par(mfrow = c(2, 2), mar = c(1, 4, 1, 4))  # Adjust margins if needed
...
# Optionally, reset the plotting layout back to default:
par(mfrow = c(1, 1))
...
pie_chart4 <- plot_ly(
  ...
  text = ~paste0("Count: ", Count),  # Show count on hover
  marker = list(colors = c("lightblue", "lightgreen", "lightcoral", "lightgray")),
  title = "SAT and ACT Participation",
...
## Supporting Analysis 1: Impact of Class Size on High School GPA




## Supporting Analysis 2: Impact of Class Size on College Readiness (ACT/SAT Scores)


## Supporting Analysis 3: Impact of Class Size on High School Graduation Rates
